    ...
    while not stop_event.is_set() :
        try :
            ...
            if not first_flag :
                ...
                # if the hand has 3 fingers raised, start a timer
                if nb_fingers == 3 :
                    first_flag = True
                    start_time = time()
            
            # if the hand has 3 fingers raised
            else :
                # if there is a change in the number of fingers raised, start over
                if nb_fingers != 3 : first_flag = False
                
                # if the hand keeps 3 fingers raised for 2 seconds, pause the video
